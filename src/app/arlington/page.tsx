...
// Icons kept in code since JSON doesn't store SVG markup. Indexed to match
// the order of items in content/arlington.json -> servicesSection.items.
const serviceIcons: ReactNode[] = [
  <svg key="refrigerator" viewBox="0 0 24 24"><rect x="4" y="2" width="16" height="20" rx="2" /><line x1="4" y1="10" x2="20" y2="10" /><line x1="10" y1="6" x2="10" y2="6.01" /><line x1="10" y1="14" x2="10" y2="14.01" /></svg>,
  <svg key="washer" viewBox="0 0 24 24"><rect x="3" y="2" width="18" height="20" rx="2" /><circle cx="12" cy="13" r="5" /><circle cx="7" cy="5" r=".8" fill="#2563eb" stroke="none" /><circle cx="9.5" cy="5" r=".8" fill="#2563eb" stroke="none" /></svg>,
  <svg key="dryer" viewBox="0 0 24 24"><line x1="4" y1="6" x2="20" y2="6" /><line x1="4" y1="12" x2="20" y2="12" /><line x1="4" y1="18" x2="20" y2="18" /><circle cx="8" cy="6" r="2" fill="#fff" /><circle cx="16" cy="12" r="2" fill="#fff" /><circle cx="10" cy="18" r="2" fill="#fff" /></svg>,
  <svg key="oven" viewBox="0 0 24 24"><path d="M12 2c-4 4-6 8-6 12a6 6 0 0 0 12 0c0-4-2-8-6-12z" /><path d="M12 22a3 3 0 0 1-3-3c0-2 1-4 3-6 2 2 3 4 3 6a3 3 0 0 1-3 3z" fill="#fff" /></svg>,
  <svg key="dishwasher" viewBox="0 0 24 24"><path d="M12 3l1.5 3.5L17 8l-3.5 1.5L12 13l-1.5-3.5L7 8l3.5-1.5L12 3z" /><path d="M5 16l1 2 2 1-2 1-1 2-1-2-2-1 2-1 1-2z" /><path d="M19 14l.75 1.5L21.5 16.25 19.75 17 19 18.5 18.25 17 16.5 16.25l1.75-.75L19 14z" /></svg>,
  <svg key="ice-machine" viewBox="0 0 24 24"><path d="M12 2v20" /><path d="M2 12h20" /><path d="M4.93 4.93l14.14 14.14" /><path d="M19.07 4.93L4.93 19.07" /></svg>,
];

export default async function ArlingtonPage() {
  const data = await getArlington();
      ...
      <main className="flex-1">
        ...
        <section className="py-16 lg:py-[72px]">
          <div className="max-w-[1160px] mx-auto px-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-[18px]">
              {data.servicesSection.items.map((s, i) => (
                <div
                  key={s.title}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-14 items-center">
              <div>
                <h2 className="text-[clamp(1.4rem,2.8vw,1.85rem)] font-bold text-slate-900 mb-4">
                  {data.areasSection.title}
                </h2>
                <p className="text-[15px] text-slate-500 leading-relaxed mb-6">
                  {data.areasSection.description}
                </p>
                <ul className="grid grid-cols-2 gap-x-6 gap-y-2.5">
                  {data.areasSection.areas.map((area) => (
        ...
        <section className="bg-blue py-16 text-center">
          <div className="max-w-[1160px] mx-auto px-6">
            ...
            <div className="flex flex-wrap justify-center gap-3">
              <a
                href={data.ctaBanner.primaryCtaHref}
              ...
              >
                {data.ctaBanner.primaryCtaLabel}
              </a>
              <Link
                href={data.ctaBanner.secondaryCtaHref}
              ...
              >
                {data.ctaBanner.secondaryCtaLabel}
              </Link>
            </div>
          </div>
        </section>
